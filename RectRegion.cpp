...
#include <iostream>
#include <vector>
RectRegion::RectRegion(std::vector<void *> edges, std::string name,
                       bool connectivityCheck, double tolerance,
                       int edgeAttractMesh, double attractEps)
    : MeshRegion(name, tolerance) {
  m_edgeAttractMesh = edgeAttractMesh;
  m_attractEps = attractEps;
  if (m_attractEps > 1.)
    m_attractEps = 1.;
  if (m_edgeAttractMesh < 0)
    m_edgeAttractMesh = 0;
  if (m_edgeAttractMesh > 3)
    m_edgeAttractMesh = 3;
  if (edges.size() < 4) {
...
RectRegion::RectRegion(std::vector<std::vector<std::vector<double>>> edges,
                       std::string name, bool connectivityCheck,
                       double tolerance, int edgeAttractMesh, double attractEps)
    : MeshRegion(name, tolerance) {
  m_edgeAttractMesh = edgeAttractMesh;
  m_attractEps = attractEps;
  if (m_attractEps > 1.)
...
std::vector<double> RectRegion::EvaluateEdgePts(int i, double s) {
  ...
  if (m_edgeType == eContinuous) {
    std::vector<double> (*edgeFunction)(double) =
        (std::vector<double>(*)(double))m_edgesFun[i];
...
int RectRegion::CheckConnectivity() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (0.5 * fabs(p11[0] - p20[0]) + 0.5 * fabs(p11[1] - p20[1]) <=
            m_tolerance ||
        0.5 * fabs(p11[0] - p21[0]) + 0.5 * fabs(p11[1] - p21[1]) <=
            m_tolerance)
      continue;

    p10 = EvaluateEdgePts(i, -s1s[i]);
    if (0.5 * fabs(p10[0] - p20[0]) + 0.5 * fabs(p10[1] - p20[1]) <=
            m_tolerance ||
        0.5 * fabs(p10[0] - p21[0]) + 0.5 * fabs(p10[1] - p21[1]) <=
            m_tolerance) {
      m_edgesDirec[i] = -1.;
    } else {
      std::cout << "error: point unconnect at edge (" << p10[0] << ", "
                << p10[1] << ":" << p11[0] << ", " << p11[1] << "), (" << p20[0]
                << ", " << p20[1] << ":" << p21[0] << ", " << p21[1]
                << std::endl;
      return -1;
    }
...
int RectRegion::ptsByIsoParametric() {
  ...
  for (int j = 0; j <= m_N; ++j) {
    ...
    for (int i = 0; i <= m_M; ++i) {
      ...
      for (int k = 0; k < 2; ++k)
        p[k] = edge0[i][k] * (1. - y) * 0.5 + edge2[i][k] * (1. + y) * 0.5 +
               edge1[j][k] * (1. + x) * 0.5 + edge3[j][k] * (1. - x) * 0.5 -
               0.25 * (1. - x) * (1. - y) * edge0[0][k] -
               0.25 * (1. + x) * (1. - y) * edge0[m_M][k] -
               0.25 * (1. - x) * (1. + y) * edge2[0][k] -
               0.25 * (1. + x) * (1. + y) * edge2[m_M][k];
      m_pts.push_back(p);
...
std::vector<double> RectRegion::EvaluateEdgePtsDerivOneSide(int i, double s) {
  ...
  for (int k = 0; k < res.size(); ++k)
    res[k] = (-147. * f[0][k] + 360. * f[1][k] - 450. * f[2][k] +
              400. * f[3][k] - 225. * f[4][k] + 72. * f[5][k] - 10. * f[6][k]) /
             (60. * h);
  return res;
}

int RectRegion::ptsByBoundaryLayer(bool trimWakeSlope, MeshType method,
                                   double offset0, double offset1) {
  // points generation
  // four vertexes + bounday points
  double dx = 2. / m_M, dy = 2. / m_N;
  ...
  double eps = 0.0001;
  for (int i = 1; i < m_M; ++i) {
    if (norm0[i][0] > norm0[i - 1][0] + eps &&
        norm0[i][0] > norm0[i - 1][0] + eps) {
      concave1 = i;
      break;
    }
  }
  ...
  double smax = EvaluateEdgePts(1, 1.)[0],
         sfirst = EvaluateEdgePts(1, -1. + 1 * dy)[0];
  if (trimWakeSlope) {
  ...
  for (int j = 0; j <= m_N; ++j) {
    for (int i = 0; i <= m_M; ++i) {
      ...
      for (int k = 0; k < 2; ++k)
        p[k] = edge0[i][k] + s * norm0[i][k];
      if (i == 0 && j) {
        p[0] += -offset0 * norm0[i][1];
        p[1] += offset0 * norm0[i][0];
        if (j == 1)
          m_offsetPts.push_back(p);
      }
      if (i == m_M && j) {
...
std::vector<double> RectRegion::getVertex(int i) { return m_vertex[i]; }
...
int RectRegion::EdgeAttractMesh(double x0, double y0, double &x, double &y) {
  ...
  x = x0;
  y = y0;
  if (m_edgeAttractMesh == 1 || m_edgeAttractMesh == 2)
    a = -a;
